...
tv_line <- function(., name = "line", width = 1, value = "value", color = "red", overlay = FALSE) {
...
tv_candles <- function(., name = "candles", value = "value", open = "open", close = "close", high = "high", low = "low", time = "time") {
  tv_add(., name, list(type = "candles", open = open, close = close, high = high, low = low, time = time))
}

tradingview(data) %>% 
  tv_chart(decimals = 2, margins = tv_margins(0,0)) %>%
  tv_line(name = "width", overlay = TRUE, width = 1, value = "Sepal.Width", color = "rgba(255,0,0,0.5)") %>%
  tv_line(name = "length", width = 1, value = "Sepal.Length", color = "rgba(0,255,0,0.5)")
```
